...
export const pushTask: Task<PushTaskInput, PushTaskOutput> = {
    async execute(input: PushTaskInput, config: Configuration, sendMessage: (message: string) => void, signal?: AbortSignal): Promise<PushTaskOutput> {
        ...
        await execTask.execute({ command: "git", args, workingDirectory: input.repoPath }, config, sendMessage, signal);
