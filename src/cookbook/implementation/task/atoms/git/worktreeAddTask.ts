...
export const worktreeAddTask: Task<WorktreeAddTaskInput, WorktreeAddTaskOutput> = {
    async execute(input: WorktreeAddTaskInput, config: Configuration, sendMessage: (message: string) => void, signal?: AbortSignal): Promise<WorktreeAddTaskOutput> {
        ...
        try {
            ...
            await worktreeRemoveTask.execute({ repoPath: input.repoPath, worktreePath: worktreePath }, config, sendMessage, signal);
        } catch {
    ...
    try {
            await execTask.execute({ command: "git", args: ["worktree", "add", worktreePath, input.branch], workingDirectory: input.repoPath }, config, sendMessage, signal);
        ...
        await execTask.execute({ command: "git", args: ["worktree", "add", "-b", input.branch, worktreePath], workingDirectory: input.repoPath }, config, sendMessage, signal);
