...
**Recommended second.** Depends on the config layer work (the `domainsPath` mechanism is a config layer addition). Medium effort but the highest structural impact — after this, a Kitchen clone has no Doma content by default.

---

## Sign-off

This report is the gate before starting the Multi-tenant Config Layer project. Findings are documented; sequencing is recommended. Ready for review.
